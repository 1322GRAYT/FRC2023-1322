...
public class RFSLIB /*extends SubsystemBase*/ {
	...
	public static double ApplyDeadBand_Scaled(double PowerRequested, double DeadBandThreshold, double deadBandLimit) {
		double powerRequestedLimited, PowerRequestedDeadBandApplied;
		double deadBandThresholdProtection;
		double Le_x_RescaleNum, Le_x_RescaleDenom;

		powerRequestedLimited = Math.min(PowerRequested, deadBandLimit);
		powerRequestedLimited = Math.max(powerRequestedLimited, -deadBandLimit);

		deadBandThresholdProtection = Math.abs(DeadBandThreshold);
		deadBandThresholdProtection = Math.min(deadBandThresholdProtection, deadBandLimit);

		if (Math.abs(powerRequestedLimited) < deadBandThresholdProtection) {
			PowerRequestedDeadBandApplied = 0;
		} else /* (Math.abs(Le_r_PwrRqstLim) >= Le_r_DB_ThrshProt) */ {
			Le_x_RescaleDenom = deadBandLimit - deadBandThresholdProtection;
			if (powerRequestedLimited >= 0.0) {
				Le_x_RescaleNum = powerRequestedLimited - deadBandThresholdProtection;
			} else /* (Le_r_PwrRqstLim < 0.0) */ {
				Le_x_RescaleNum = powerRequestedLimited + deadBandThresholdProtection;
			}
			PowerRequestedDeadBandApplied = (Le_x_RescaleNum / Le_x_RescaleDenom) * deadBandLimit;
		}

		return (PowerRequestedDeadBandApplied);
	}

	public static double ApplyDeadBand_Zeroed(double PowerRequested, double DeadBandThreshold) {
		double powerRequestedLimited, PowerRequestedDeadBandApplied;
		double deadBandThresholdProtection;

		powerRequestedLimited = Math.min(PowerRequested, 1.0);
		powerRequestedLimited = Math.max(powerRequestedLimited, -1.0);

		deadBandThresholdProtection = Math.abs(DeadBandThreshold);
		deadBandThresholdProtection = Math.min(deadBandThresholdProtection, 1.0);

		if (Math.abs(powerRequestedLimited) < deadBandThresholdProtection) {
			PowerRequestedDeadBandApplied = 0;
		} else /* (Math.abs(Le_r_PwrRqstLim) >= Le_r_DB_ThrshProt) */ {
			if (powerRequestedLimited >= 0.0) {
				PowerRequestedDeadBandApplied = powerRequestedLimited - deadBandThresholdProtection;
			} else /* (Le_r_PwrRqstLim < 0.0) */ {
				PowerRequestedDeadBandApplied = powerRequestedLimited + deadBandThresholdProtection;
			}
		}
	...
	public static double ApplyDeadBand_Clipped(double PowerRequested, double DeadBandThreshold) {
		double PowerRequestedDeadBandApplied;
		double deadBandThresholdProtection;

		deadBandThresholdProtection = Math.abs(DeadBandThreshold);

		if (Math.abs(PowerRequested) < deadBandThresholdProtection) {
			PowerRequestedDeadBandApplied = 0;
		} else /* (Math.abs(Le_r_PwrRqstRaw) >= Le_r_DB_ThrshProt) */ {
			PowerRequestedDeadBandApplied = PowerRequested;
		}
	 ...
	 */
	public static double[] NormalizeArrayValues(double La_x_ValDsrd[], double Le_x_ValNormMax) {
	 ...
	 */
	public static double LimRateOnIncrease(double ValRaw, double ValLimPrev, float DeltLimMax) {
		double DeltUpd;
		double ValTemp;
		double ValLimNew;
		boolean LimPosInc;
	...
	public static float AxisPieceWiseLinear_int(float InpVal,
		...
		if (InpValInt <= AxisArray[0]) {
			...
			for (ArrayIdx = 1; InpValInt >= AxisArray[ArrayIdx]; ArrayIdx++) {
			}

			ArrayIdx--;

			InterpFrac = InterpolationCoefficientFraction(InpVal,
	...
	public static float AxisPieceWiseLinear_flt(float InpVal,
		...
		if (InpVal <= AxisArray[0]) {
			...
			for (ArrayIdx = 1; InpVal >= AxisArray[ArrayIdx]; ArrayIdx++) {
			}
	...
	public static float AxisLinear_flt(float InpVal,
		...
		if (InpVal <= LwrRef) {
			...
			AxisOutVal = InterpolationCoefficientFraction(InpVal, LwrRef, UprRef);
		}
	 ...
	 */
	private static float InterpolationCoefficientFraction(float Inp,
			float LwrRef,
			float UprRef) {
	 ...
	 * @return: Fractional Interpolated Output Value (float)
	 */
	// todo: lambda expression - need to learn how to add a label to reference the
	// expression
	/* (Inp, LwrRef, UprRev) -> ((Inp - LwrRef)/(UprRef - LwrRef)); */

	/**
	 * Method: LinearInterp_flt - This function will return the Linear Interpolation
	 * of two given points (Lower and Upper Reference) given a fractional
	 * coefficient
	 * factor scalar (0 - 1)
	 * 
	 ...
	 * 
	 * @param: Look-Up    Table Array Object Reference (reference to array of
	 *                    floats)
	 ...
	 * table look-up value based on a rescaled normalized axis index value.
	 * 
	 * @param1: Look-Up Table Array Object Reference (reference to 3-D array of
	 *          floats)
